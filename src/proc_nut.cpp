...
{	
	printf_log ("illegal opcode %03x at %04x\n", opcode, nut_reg->prev_pc);
}
...
{	
	...
	do_event_int (nut_reg, (event_t) event_ram_select);
...
{	
	...
	do_event_int (nut_reg, (event_t) event_periph_select);
...
static void op_read_reg_n (nut_reg_t *nut_reg, int opcode)
{
    ...
    {
		if (is_pf)
			printf_log ("warning: conflicting read RAM %03x PF %02x reg %01x\n",
    ...
    {
		printf_log ("warning: stray read RAM %03x PF %02x reg %01x\n",
				ram_addr, pf_addr, opcode >> 6);
    }
}
...
static void op_write_reg_n (nut_reg_t *nut_reg, int opcode)
{
    ...
    {
		if (is_pf)
			printf_log ("warning: conflicting write RAM %03x PF %02x reg %01x\n",
					ram_addr, pf_addr, opcode >> 6);
	...
	else if (! is_pf)
    {
		...
		printf_log ("warning: stray write RAM %03x PF %02x reg %01x data ",
				ram_addr, pf_addr, opcode >> 6);
		print_reg (nut_reg->c);
		printf_log ("\n");
#endif
...
static void op_c_to_data (nut_reg_t *nut_reg, int opcode)
{
    ...
    {
		if (is_pf)
			printf_log ("warning: conflicting write RAM %03x PF %02x\n",
					ram_addr, pf_addr);
		if (nut_reg->ram_write_fn [ram_addr])
			nut_reg->ram_write_fn [ram_addr] (nut_reg, ram_addr, & nut_reg->c);
		else
    ...
    {
#ifdef WARN_STRAY_WRITE
		printf_log ("warning: stray write RAM %03x PF %02x data ",
				ram_addr, pf_addr);
		print_reg (nut_reg->c);
		printf_log ("\n");
#endif
...
{	
#ifdef KEYBOARD_DEBUG
	printf_log ("kb reset, addr %04x, state %s\n", nut_reg->prev_pc, kbd_state_name [nut_reg->kb_state]);
#endif
...
{
#ifdef KEYBOARD_DEBUG
	printf_log ("nut_kbd_scanner_sleep, state=%s\n", kbd_state_name [nut_reg->kb_state]);
#endif
...
bool nut_execute_cycle (nut_reg_t *nut_reg)
{
	...
	switch (nut_reg->inst_state)
    {
		...
		case selprf:
			...
			break;
		default:
			printf_log ("nut: bad inst_state %d!\n", nut_reg->inst_state);
			nut_reg->inst_state = norm;
			break;
    }
	nut_reg->cycle_count++;
...
{	
#ifdef KEYBOARD_DEBUG
	printf_log ("key release, addr %04x, state %s\n", nut_reg->prev_pc, kbd_state_name [nut_reg->kb_state]);
#endif
